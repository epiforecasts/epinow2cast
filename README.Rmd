...
* `estimate_infections()`: Reconstruct cases by date of infection from reported cases using a renewal equation model with random walk priors on the time-varying reproduction number.

* `simulate_infections()`: Simulate infections from a given reproduction number trajectory.

The default model in `estimate_infections()` uses a random walk prior to estimate the time-varying reproduction number and infer infections. It accounts for reporting delays, day-of-week effects, and observation overdispersion (Poisson or negative binomial).

Since version 2.0.0, `{EpiNow2}` uses the [`{epinowcast}`](https://package.epinowcast.org/) package as its computational backend, replacing the previous Stan-based implementation. Models are fit with [MCMC sampling](https://mc-stan.org/docs/reference-manual/mcmc.html) via [`cmdstanr`](https://mc-stan.org/cmdstanr/).
...
`{EpiNow2}` requires [`cmdstanr`](https://mc-stan.org/cmdstanr/) and a working CmdStan installation. See the [cmdstanr documentation](https://mc-stan.org/cmdstanr/articles/cmdstanr.html) for installation instructions.
